...
def calc_score(er, baff, k_pct, p7, p30, mm_ok, ao_pos, cross, trend,
    ...
    if vortex_bull: s += 5
    if rvi_bull:    s += 5
    # RS Line rimossa: non rilevante per ETF geografici
...
def eval_buy_level(c, vix_regime):
    ...
    # BUY3 — RS Line rimossa: non rilevante per ETF geografici
    # Sostituita da: Vortex bull + RVI bull + baffetti ≥ 3
    ...
    # BUY1 — cross KAMA deve essere recente (≤ 5 barre)
...
def eval_exit(pos, cur):
    """
    Valuta EXIT e DOWNGRADE.
    Ritorna (action, reason) dove action è:
      EXIT3, EXIT1b, EXIT2, EXIT1, DOWNGRADE_BUY2, DOWNGRADE_BUY1, None
    """
    score         = cur["score"]
    peak_score    = pos.get("peak_score", score)
    # Fallback peak_score: se manca usa score+8 per evitare EXIT2 immediato
    if peak_score == score:
        peak_score = score + 8
    er            = cur["er"]
    ao_val        = cur["ao"]
    ...
    # ── EXIT3 — soglia score dipende dal livello ──────────────────────────
    ...
    # ── EXIT1 — parziale: SAR gira OR AO cala 3+ sessioni ────────────────
...
def update_portfolio(existing, history, cooldowns, candidates, today_str, vix_regime):
    ...
    for pos in existing:
        ...
        # Se events è vuoto (posizione pre-v5), logga evento iniziale
        if not events:
            ev0 = make_event(
                current_level,
            ...
            # Usa entry_ts se disponibile, altrimenti entry_date
        ...
        if exit_level == "DOWNGRADE_BUY2":
            ...
            # Uscita parziale — mantieni posizione
            last_lvl = events[-1]["level"] if events else None
            ...
            # Nessun exit/downgrade: verifica upgrade
