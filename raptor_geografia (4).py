...
def calc_rsi_series(prices, n=14):
    """Calcola RSI come serie completa — per il grafico."""
    if len(prices) < n + 1:
...
def classify_regime(h60, h1y, adx, pdi, ndi):
    if adx >= 25:
        if pdi >= ndi:
            if h60 > 0.55:
                return {"code":"SLANCIO","label":"🚀 Slancio","color":"#1a7f37","desc":"Trend rialzista forte con memoria recente confermata"}
            else:
                return {"code":"SALITA","label":"📈 Salita","color":"#2ea043","desc":"Movimento rialzista in corso"}
        else:
            return {"code":"RIBASSO","label":"📉 Ribasso","color":"#cf222e","desc":"Movimento ribassista in corso"}
    elif adx >= 20:
        return {"code":"TRANSIZIONE","label":"⚠️ Transizione","color":"#bc4c00","desc":"Zona grigia, segnale debole"}
    else:
        return {"code":"LATERALE","label":"↔ Laterale","color":"#8c98a4","desc":"Mercato senza direzione"}

# ─────────────────────────────────────────────────────────────────────────────
# CHART JSON — genera data/charts/TICKER.json per il grafico HTML
# ─────────────────────────────────────────────────────────────────────────────

def save_chart_json(ticker, closes, highs, lows, dates,
                    ao_series_full, kama_series,
                    sar_series, sar_bull_series, rsi_series,
                    positions_events=None):
    """
    Salva data/charts/TICKER.json con tutte le serie per il grafico HTML.
    Standard RAPTOR — compatibile con scannerv2.
    """
    ...
    signals_history = []
    if positions_events:
        for ev in reversed(positions_events[-10:]):
...
def analyze(ticker, name, bench_close, positions_events=None):
    try:
        ...
        # ── Calcola serie complete per il grafico ─────────────────────────
        rsi_series_full = calc_rsi_series(close)

        # AO serie completa (aggiungi padding iniziale)
        ...
        try:
            save_chart_json(
                ...
                positions_events = positions_events,
            )
...
def update_portfolio(existing, history, cooldowns, candidates, today_str, vix_regime):
    ...
    for pos in existing:
        ...
        if exit_level == "DOWNGRADE_BUY2":
            ...
            if last_lvl != "DOWNGRADE_BUY2":
                events.append(make_event("DOWNGRADE_BUY2", exit_reason,
            ...
            if last_lvl != "DOWNGRADE_BUY1":
                events.append(make_event("DOWNGRADE_BUY1", exit_reason,
            ...
            if last_lvl != exit_level:
                events.append(make_event(exit_level, exit_reason,
    ...
    new_cands = sorted(
        [c for c in candidates if c["ticker"] not in in_port and c["ticker"] not in cooldowns],
...
def main():
    ...
    # Crea cartella chart se non esiste
    os.makedirs("data/charts", exist_ok=True)
    ...
    # Mappa events per ticker (per la storia segnali nel grafico)
    def get_events(ticker, positions):
        pos = next((p for p in positions if p["ticker"] == ticker), None)
        return pos.get("events", []) if pos else None

    print("\n📊 PAESI...")
    paesi_data = []
    for t, n in PAESI.items():
        print(f"  {t}...", end=" ", flush=True)
        evts = get_events(t, state["paesi"].get("positions", []))
        r = analyze(t, n, bench, positions_events=evts)
        if r:
            paesi_data.append(r)
            print(f"✓ score={r['score']} H60={r['hurst_60']} ADX={r['adx']} → {r['regime']['code']}")
        else:
            print("✗")
    ...
    for t, n in NEW_AREA.items():
        print(f"  {t}...", end=" ", flush=True)
        evts = get_events(t, state["new_area"].get("positions", []))
        r = analyze(t, n, bench, positions_events=evts)
        if r:
    ...
    n_charts = len([f for f in os.listdir("data/charts") if f.endswith(".json")]) if os.path.exists("data/charts") else 0
    ...
    print(f"   CHARTS   : {n_charts} file JSON in data/charts/")
