...
Regime Hurst+ADX — solo visivo, nessun impatto sui segnali
...
def calc_adx_simple(high, low, close, n=14):
    """ADX semplificato inline — restituisce (adx, pdi, ndi)."""
    if len(close) < n + 2:
...
def classify_regime(h, adx_val, pdi_val, ndi_val):
    ...
    if h > 0.55 and adx_val > 25:
        ...
        return {
            ...
            "label":       "↔ Mean Rev",
            "color":       "#378ADD",
            "description": "Serie anti-persistente, tende a tornare alla media"
        }
        ...
        return {
            ...
            "label":       "~ Incerto",
            "color":       "#888780",
            "description": "Segnali contrastanti tra Hurst e ADX"
        }
...
def analyze(ticker, name, bench_close):
    try:
        ...
        adx_val, pdi_val, ndi_val = calc_adx_simple(high, low, close)
        regime = classify_regime(hurst_val, adx_val, pdi_val, ndi_val)
        # ── Fine regime ───────────────────────────────────────────────────────
...
def main():
    ...
    for t, n in PAESI.items():
        ...
        if r:
            paesi_data.append(r)
            print(f"✓ score={r['score']} er={r['er']:.2f} H={r['hurst']} regime={r['regime']['code']}")
        else:
            print("✗")
