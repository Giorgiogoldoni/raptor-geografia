#!/usr/bin/env python3
"""
🦅 RAPTOR GEOGRAFIA v3 — ETF UCITS Europa | PAESI + NEW AREA
Aggiornamento orario 09:00-19:00 CET via GitHub Actions
"""
...
NEW_AREA = {
    ...
    # ── World ex USA ──────────────────────────────────────────────────────────
    "EXUS.DE":  "Xtrackers MSCI World ex USA UCITS ETF",
    "WEXE.DE":  "Amundi MSCI World ex USA UCITS ETF Acc",
    "XUSE.AS":  "iShares MSCI World ex-USA UCITS ETF USD Acc",
}
...
def calc_supertrend(high, low, close, period=10, mult=3.0):
    ...
    for i in range(period, len(tr)):
        atr_l.append((atr_l[-1]*(period-1)+tr[i])/period)
...
def analyze(ticker, name):
    try:
        ...
        return {
            ...
            "price":round(price,4), "kama":round(kama_v,4), "k_pct":k_pct,
            "er":er, "ao":round(float(ao),6), "baffetti":baff, "rsi":rsi_v,
            "atr":round(atr_v,5), "trailing_stop":trail,
...
def update_portfolio(existing, candidates, today_str):
    ...
    for pos in existing:
        ...
        pos.update({
            ...
            "sar_bullish":cur["sar_bullish"],"perf7":cur["perf7"],"perf30":cur["perf30"],
            "warning":None,
        })
        kept.append(pos)
    slots=5-len(kept)
    ...
    for c in new_q[:slots]:
        ...
        kept.append({
            ...
            "supertrend_bullish":c["supertrend_bullish"],"sar_bullish":c["sar_bullish"],
            "perf7":c["perf7"],"perf30":c["perf30"],"weight_pct":0,"warning":None,
        })
...
def main():
    ...
    print(f"🦅 RAPTOR GEOGRAFIA v3 — {now.strftime('%d/%m/%Y %H:%M')} CET")
    state=load_state()
    ...
    output={
        ...
        "paesi":{
            "positions":paesi_pos,"use_xeon":len(paesi_pos)==0,
        ...
        "new_area":{
            "positions":new_area_pos,"use_xeon":len(new_area_pos)==0,
            "xeon_price":xeon["price"] if xeon else None,
